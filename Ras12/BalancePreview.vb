...
Public Class BalancePreview
    ...
    Private Sub LoadGridBalance()
        ...
        If Me.OptFT.Checked Then
            strSQL = strSQL & " and c.CustID in (select CustID from officeID where status<>'XX' and OID_Usage like '%FT%')"
    ...
    Private Sub LoadGridDetail()
        ...
        strSQL = strSQL & " from cc_BLC where custID=" & Me.GridBalance.CurrentRow.Cells("CustID").Value &
            " and FstUpdate between '" & Format(Me.txtFrm.Value, "dd-MMM-yy") & "' and '" &
            Format(Me.TxtTo.Value, "dd-MMM-yy 23:59") & "' order by Fstupdate Desc, RecID desc"
    ...
    Private Sub GridBalance_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridBalance.CellContentClick
