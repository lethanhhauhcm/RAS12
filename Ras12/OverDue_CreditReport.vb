Public Class OverDue_CreditReport
    ...
    Private Sub LoadGridOverDueCredit()
        ...
        Me.GridOverLimit.DataSource = GetDataTable("select custID, CustShortName as CustName," &
                                                   "case PPCoef when 0 then VND_PSP_Avail else VND_PPD_Avail end as CurrBLC," &
                                                    " getdate() as LstPmt from cc_BLC  " &
                                                    " where City='" & myStaff.City & "' and RecID in (Select Max(RecID) From CC_BLC where city='" _
                                                    & myStaff.City & "' Group by CustID) " &
                                                    strDKCustType &
