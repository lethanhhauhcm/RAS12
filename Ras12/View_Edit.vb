...
Public Class View
    ...
    Private Sub SearchByDate()
        ...
        strSQL = strSQL & " RPTNO, PrintedCustName, PrintedCustAddrr, PrintedTaxCode, FstUser, FstUpdate,Vendor,VendorId from RCP "
        strSQL = strSQL & " where status <>'XX' and FstUpdate between '" & Me.txtFrmDate.Text
        strSQL = strSQL & "' and ' " & Me.txtToDate.Text & " 23:59' and Counter='" & myStaff.Counter & "'"
        'If tmpBiz = "GSA" Then
        '    strSQL = strSQL & " and al='" & Me.CmbAL.Text & "' "
        'End If
        Try
            If Me.cboSearchType.Text <> "" Then
                strSQL = strSQL & " and RecId in (select RcpId from Tkt where Status<>'XX' and " _
                    & cboSearchType.Text & " like '%" & txtSearchValue.Text & "%')"
            End If
    ...
    Private Sub SearchSpecific()
        ...
        If CmbFindWhat.Text = "" Then
            MsgBox("You must specify type of record")
            Exit Sub
        End If
        If Me.CmbFindWhat.Text.Substring(0, 3) = "TKT" Then
            DK = " RecID in (select RCPID from TKT "
            DK = DK & " where Replace(TKNO,' ','')='" & Replace(cboSearchType.Text, " ", "") _
                & "' and (tkt.Statusal<>'xx' or tkt.Status<>'xx'))"
        ElseIf Me.CmbFindWhat.Text.Substring(0, 3) = "TRX" Then
            DK = " RCPNO='" & Me.cboSearchType.Text & "'"
        ElseIf Me.CmbFindWhat.Text.Substring(0, 3) = "RPT" Then
            DK = " RPTNO='" & Me.cboSearchType.Text & "'"
        ElseIf Me.CmbFindWhat.Text = "TCODE" Then
            DK = " DeliveryStatus='" & Me.cboSearchType.Text & "'"
        Else
            DK = " RecID in (select RCPID from TKT "
            DK = DK & " where TKNO like '%TV%' and dependent='" & Me.cboSearchType.Text & "')"
        End If
        'custid
        strSQL = "select RecID, RCPNO as TRX, SRV, CustType as Channel, CustID, CustShortName as CustName,"
        strSQL = strSQL & " Status, TTLDue, Currency as Curr, ROE, Charge, Discount, DOS, Counter,  Location,"
        ...
        '    strSQL = strSQL & " and CustShortName<>'TVREFUND'"
        'End If
        Me.GridRCP.DataSource = GetDataTable(strSQL)
    ...
    Private Sub ReSizeColR(ByVal pFOP As Int16)
        ...
        For i As Int16 = 0 To Me.GridRCP.ColumnCount - 1
            ...
            If InStr(strColName, "ID") > 0 Or InStr(strColName, "User") > 0 Or
                strColName = "Type" Or strColName = "Channel" Or strColName = "Curr" Or
        ...
        For i As Int16 = 0 To Me.GridFOP.ColumnCount - 1
            ...
            If InStr(strColName, "ID") > 0 Or InStr(strColName, "User") > 0 Or InStr(strColName, "Status") > 0 Or
    ...
    Private Sub GridRCP_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridRCP.CellContentClick
        ...
        If Me.GridRCP.CurrentRow.Cells("Counter").Value = "N-A" Then
            SQLstr = " Select * From Dutoan_item where Status='OK' and DutoanId= (select RecId from Dutoan_tour where RCPID=" & Me.GridRCP.CurrentRow.Cells("RecID").Value & ")"
        Else
            SQLstr = " Select * From Tkt where Status<>'XX' and RCPID=" & Me.GridRCP.CurrentRow.Cells("RecID").Value
        End If
        dgrTransactions.DataSource = GetDataTable(SQLstr)
        dgrTransactions.AutoResizeColumns()
        dgrTransactions.Visible = True
    End Sub
    ...
    Private Sub cmbValueToSearch_Enter(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboSearchType.Enter
        If Me.cboSearchType.Text = "CustID" Then
            Me.cboSearchType.Text = 0
            Me.cboSearchType.ForeColor = Color.Black
        End If
    End Sub
    Private Sub cmbValueToSearch_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboSearchType.LostFocus
        If Me.cboSearchType.Text.Length < 13 Then Exit Sub
        If Me.cboSearchType.Text.Substring(0, 1) = "Z" Then Exit Sub
        If Me.CmbFindWhat.Text.Substring(0, 3) = "TKT" And Me.cboSearchType.Text.Length = 13 Then
            Me.cboSearchType.Text = AddSpace2TKNO(Me.cboSearchType.Text)
        End If
    End Sub
    Private Sub CmbFindWhat_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmbFindWhat.LostFocus
        Me.cboSearchType.Items.Clear()
        If Me.CmbFindWhat.Text = "RPT No." Then
            ...
            Dim strQry As String = strDK & " and dos >'" & Format(ngayCuoiThangTruoc, "dd-MMM-yy") & "' UNION " & strDK &
                " and dos between '" & Format(ngayDauThangNay.AddMonths(-1), "dd-MMM-yy") & "' and '" &
                Format(ngayCuoiThangTruoc, "dd-MMM-yy") & "' order by RPTNO desc "
            LoadCmb_MSC(Me.cboSearchType, strQry)
        End If
    End Sub
    ...
    Private Sub CmbFindWhat_VisibleChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmbFindWhat.VisibleChanged
        ...
        If CmbFindWhat.Visible Then
            Me.cboSearchType.Left = 285
            Me.cboSearchType.Width = 132
            Me.cboSearchType.Text = ""
            Me.cboSearchType.ForeColor = Color.Black
        Else
            Me.cboSearchType.Left = 365
            Me.cboSearchType.Width = 56
            'Me.cboSearchType.Text = "CustID"
            Me.cboSearchType.ForeColor = Color.Gray
        End If
    End Sub
    ...
    Private Sub OptBySearch_CheckedChanged(sender As Object, e As EventArgs) Handles OptBySearch.CheckedChanged
        If OptBySearch.Checked Then
