...
Public Class GroupBooking
    ...
    Private Sub GroupBooking_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        LoadCmb_VAL(Me.CmbCust, "select RecID as VAL, CustShortName as DIS from customerlist" _
                    & " where City='" & myStaff.City & "' and recID in " _
                    & "(select custID from cust_detail where status='OK' and Cat='Channel' and val in ('TA','TO'))" &
                     " and recID>0 order by CustShortName")
        LoadCmb_MSC(Me.CmbAL, "select AL as VAL from airline where AL not in ('01','XX') and City='" & myStaff.City & "' order by AL")
        LoadCmb_MSC(Me.CmbAL_Mail, "select AL as VAL from airline where AL not in ('01','XX') and City='" & myStaff.City & "' order by AL")
        Me.CmbCurr.Text = "USD"
        Me.CmbStatus.Text = "OK"
        Me.CmbPackg.Text = "Series"
        LoadGridRQ("")
        ClearScreen()
        cboFilterDeadline.SelectedIndex = -1
    ...
    Private Sub InsertNewPNR(pRQNo As String, pCustID As Integer)
        ...
        cmd.CommandText = "insert ActionLog (TableName, doWhat, ActionBy, F1, F2, F3, F4, F5, F6, F7, F8,F9,F10, F11,F12, F13,City) " &
            "values ('GRPBKG','OK','" & myStaff.SICode & "', @F1, @F2, @F3, @F4, @F5, @F6, @F7, @F8,@F9,@F10,@F11,@F12,@F13,@City) ;SELECT SCOPE_IDENTITY() AS [RecID]"
        cmd.Parameters.Clear()
        'custid
        ...
        cmd.Parameters.Add("@F12", SqlDbType.VarChar).Value = Format(Me.txtDOF.Value, "dd-MMM-yy")
        ...
        cmd.Parameters.AddWithValue("@City", myStaff.City)

        cmd.CommandText = "insert ActionLog (TableName, doWhat, actionBy, F1, F2, F3, F4, F5,City) " &
            "values ('GRPBKGDL','OK','" & myStaff.SICode & "', @F1, @F2, @F3, @F4, @F5,@City)"
        ...
        'cmd.CommandText = "insert ActionLog (TableName, doWhat, actionBy, F1, F2, F3, F4, F5,City) " &
        '    "values ('GRPBKGDL','OK','" & myStaff.SICode & "'," & recid & ",'TKTG','" _
        '      & Format(Me.txtTktg.Value, "dd-MMM-yy") & "','??','" & myStaff.City & "')"
        'cmd.ExecuteNonQuery()
        ...
        'cmd.Parameters.AddWithValue("@City", myStaff.City)
        'cmd.Parameters.AddWithValue("@F6", "")
        cmd.ExecuteNonQuery()
        LoadGridRQ("")
    ...
    Private Sub LoadGridRQ(pRQNo As String)
        ...
        End If
        If chkFilterByDOF.CheckState = CheckState.Checked Then
            strQry = strQry & " and F12<>'' and cast(F12 as smalldatetime) between '" _
                & CreateFromDate(dtpFrom.Value) _
    ...
    Private Sub LoadGridRLOC(pRQ As String, pRLOC As String)
        ...
        Me.GridRLOC.DataSource = GetDataTable("select RecID, F1, F2,F3, F4, F5, F6, F7, F8,F9, F10, '' as RLOC" _
                                ...
                                & " from actionLog where tableName='GRPBKG' and doWhat='OK' and " _
                                & " City='" & myStaff.City & "' and " & strDK)
        ...
        If GridRLOC.RowCount > 0 Then
            GridRLOC.Rows(0).Selected = True
        End If
    End Sub
    ...
    Private Sub LblDeleteRQ_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblDeleteRQ.LinkClicked
        ...
        cmd.CommandText = "update ActionLog set doWhat='XX', F12='" & myStaff.SICode & "|" & Format(Now, "dd-MMM-yy HH:mm") &
            "' where tableName='GRPBKG' and doWhat='OK' and F2='" & SelectedRQ _
            & "' and City='" & myStaff.City & "'" _
            & "; update ActionLog set dowhat='XX', F12='" & myStaff.SICode & "|" & Format(Now, "dd-MMM-yy HH:mm") &
                "' where tableName='GRPBKGDL' and F1 in (select RecID from actionlog where " &
                " tableName='GRPBKG' and F2='" & SelectedRQ & "') and City='" & myStaff.City & "'"
        cmd.ExecuteNonQuery()
        LoadGridRQ("")
    End Sub

    Private Sub LblDone_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblDone.LinkClicked
        cmd.CommandText = "update ActionLog set doWhat='RR', F12='" & myStaff.SICode & "|" & Format(Now, "dd-MMM-yy HH:mm") &
            "' where tableName='GRPBKG' and doWhat='OK' and F2='" & SelectedRQ & "'" &
            "; update ActionLog set F4='OK', F12='" & myStaff.SICode & "|" & Format(Now, "dd-MMM-yy HH:mm") &
                "' where tableName='GRPBKGDL' and F1 in (select RecID from actionlog where " &
                " tableName='GRPBKG' and F2='" & SelectedRQ & "')  and City='" & myStaff.City & "'"
        cmd.ExecuteNonQuery()
        LoadGridRQ("")
    ...
    Private Sub LoadGridDeadline()
        ...
        Try
            ...
            Dim strQry As String = "select d.RecID, d.F1 as RLOCID,'' as RequestNo, '' as RLOC" _
                                    ...
                                    & "' and  g.doWhat='OK' and d.City='" & myStaff.City _
                                    & "' and g.City='" & myStaff.City & "'"

            'If Not Me.chkAllRQST.Checked AndAlso GridRLOC.CurrentRow IsNot Nothing Then
            '    strQry = strQry & " and d.F1='" & Me.GridRLOC.CurrentRow.Cells("RecID").Value.ToString & "'"
            'End If
            If cboFilterDeadline.Text = "Past and Comming" Then
            ...
            For r As Int16 = 0 To Me.GridDL.RowCount - 1
                ...
                If Me.GridDL.Item("RLOCID", r).Value <> 0 Then
                    Me.GridDL.Item("RLOC", r).Value = ScalarToString("ActionLog", "f3", "RecID=" & Me.GridDL.Item("RLOCID", r).Value _
                                                                    & " and City='" & myStaff.City & "'").ToString.Substring(0, 6)
                    Me.GridDL.Item("RequestNo", r).Value = ScalarToString("ActionLog", "f2", "RecID=" & Me.GridDL.Item("RLOCID", r).Value _
        ...
        End Try
    End Sub
    Private Sub txtFareADL_LostFocus(sender As Object, e As EventArgs) Handles txtFareADL.LostFocus, txtFareCHD.LostFocus,
    ...
    Private Sub LblDone_DL_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblDone_DL.LinkClicked
        cmd.CommandText = "update ActionLog set F4='OK', F12='" & myStaff.SICode & "|" & Format(Now, "dd-MMM-yy HH:mm") & "'" &
    ...
    Private Sub LblChangeDL_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblPostpone.LinkClicked
        ...
        cmd.CommandText = "update ActionLog set F3='" & Format(Me.txtMyDeadline.Value, "dd-MMM-yy") & "',F5='" & Me.txtTask.Text &
            "', F12='" & myStaff.SICode & "|" & Format(Now, "dd-MMM-yy HH:mm") & "'" &
            " where RecID=" & Me.GridDL.CurrentRow.Cells("RecID").Value
        cmd.ExecuteNonQuery()
        LoadGridDeadline()
    End Sub
    Private Sub LblAddDL_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblAddDL.LinkClicked
        cmd.CommandText = "insert ActionLog (TableName, doWhat, actionBy, F1, F2, F3, F4, F5) " &
            "values ('GRPBKGDL','OK','" & myStaff.SICode & "', @F1, @F2, @F3, @F4, @F5,@City)"
        cmd.Parameters.Clear()
        If cboFilterDeadline.Text = "ALL" Then
    ...
    Private Sub LblEmail_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblEmail.LinkClicked
        Dim txtSubj As String = Me.GridRQ.CurrentRow.Cells("RequestNo").Value & ": " &
            Me.GridRLOC.CurrentRow.Cells("F3").Value.ToString.Split("|")(3) &
        ...
        txtBody = txtBody & "%0d" & "Please find below details for your group booking request" & "%0d" & "%0d" &
            Me.GridRLOC.CurrentRow.Cells("F9").Value.ToString.Replace(vbLf, "%0d")
        Dim txtEmail As String = ""
        'custid
        Dim dTbl As DataTable = GetDataTable("select distinct VAL from Cust_detail where cat='EMAIL' and status='OK' and CustID=" &
    ...
    Private Sub lblEmailAL_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lblEmailAL.LinkClicked
        ...
        txtBody = txtBody & "%0d" & "Please " & "%0d" & "%0d" &
            Me.GridRLOC.CurrentRow.Cells("F9").Value.ToString.Replace(vbLf, "%0d")
    ...
    Private Sub CmbCust_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CmbCust.SelectedIndexChanged
        If ChkSelectCustOnly.CheckState = CheckState.Checked Then
            LoadGridRQ("")
        End If
    End Sub

    Private Sub chkFilterByDOF_CheckedChanged(sender As Object, e As EventArgs) Handles chkFilterByDOF.CheckedChanged
        LoadGridRQ("")
    End Sub

    Private Sub cboFilterDeadline_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboFilterDeadline.SelectedIndexChanged
