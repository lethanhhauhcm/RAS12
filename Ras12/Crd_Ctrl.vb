...
Public Class Crd_Ctrl
    Public Shared Function defineVND_Avail(ByVal pCustID As Integer, ByVal parType As String, ByVal parCutOverDate As Date _
                                           , ByVal parCutOverDateNew As Date, ByVal pRMK As String _
                                           , ByVal ppConn As SqlClient.SqlConnection, ByVal pSICode As String _
                                           , ByVal pCnString As String) As Decimal
        ...
        strSQL = "Insert cc_BLC (FstUser,City, CustShortName, CustID, CRCoef, PPCoef, FT_Sale, SabreSale, "
        If parType = "PSP" Then
            strSQL = strSQL & " PSP_UnUsed, PSP_UnInv, PSP_UnPaid, BG, CR "
        ElseIf parType = "PPD" Then
            strSQL = strSQL & " PPD_Depo, PPD_Used"
        End If
        strSQL = strSQL & ", RMK) select '" & pSICode & "','" & myStaff.City & "','" & CustName & "', custID, CrCoef, PPCoef," & FT_Sale
        ...
        End If
        cmd.CommandText = strSQL & " from cc_BLC where custid=" & pCustID & " and City='" & myStaff.City & "' order by recid desc"
    ...
    Private Shared Function Define_SaleNotInRAS(ByVal pCustID As Integer, ByVal ppConn As SqlClient.SqlConnection, ByVal pFT_1S As String) As Decimal
        ...
        If pFT_1S = "FT" Then
            PRGlist = "('TTQ','TTP','A1S','AKW')"
        Else
            PRGlist = "('M1S')"
        End If
        Dim cmd As SqlClient.SqlCommand = ppConn.CreateCommand
        cmd.CommandText = "select isnull(sum(CreditAmt*ROE*qty),0) from SalesNotInRas where CustID=" & pCustID _
            & " and PRG in " & PRGlist
        cmd.CommandTimeout = 128
        KQ = cmd.ExecuteScalar
        Return KQ
    End Function
    Private Shared Function DefineBG_TinChap(ByVal pCust As Integer, ByVal BG_CR As String, ByVal ppConn As SqlClient.SqlConnection) As Decimal
        ...
        strSQL = strSQL & " and '" & Now.Date & "' between BGValidFrm and BGExpireDate "
        If BG_CR = "BG" Then
    ...
    Private Shared Function DefineUnUse(ByVal pCust As Integer, ByVal ParPmtType As String, ByVal pCutOverDateNew As Date, ByVal pCutOverDate As Date, pppConn As SqlClient.SqlConnection) As Decimal
        ...
        'cmd1.CommandText = "select isnull(sum(conlai*ROE),0) as Conlai from TT_KhachTra where status <>'XX' and " &
        '    "PmtType ='" & ParPmtType & "' and CustID=" & pCust & " and isClosed=0 and fstUpdate >'" & pCutOverDateNew &
        '    "' group by OrgCurr"
        cmd1.CommandText = "select isnull(sum(conlai*ROE),0) as Conlai from KhachTra where status <>'XX' and " &
            "PmtType ='" & ParPmtType & "' and CustID=" & pCust & " and isClosed=0 and fstUpdate >'" _
            & pCutOverDateNew & "'"
    ...
    Private Shared Function DefineUnInvoice(ByVal pCust As Integer, ByVal ParPmtType As String, ByVal pCutOverDateNew As Date, ByVal pCutOverDate As Date, pppConn As SqlClient.SqlConnection) As Decimal
        ...
        If pCutOverDateNew = "00:00" Then pCutOverDateNew = pCutOverDate
        Dim adapter As New SqlClient.SqlDataAdapter("select SRV, isnull(sum(Amount*ROE),0) as Amt from func_CC_PSP " & _
                    "(" & pCust & ",'" & pCutOverDateNew & "','" & ParPmtType & "')  group by SRV", pppConn)
