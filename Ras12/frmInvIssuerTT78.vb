...
Public Class frmInvIssuerTT78
    ...
    Public Sub New(ByVal parWhois As String, parRCPNO As String)
        ...
        lbkCurrentTS.Text = "TS" & Format(Now, "MMyy00")
    End Sub
    Private Sub LoadTKT(ByVal ParDKRCP As String, Optional blnIgnoreDupCheck As Boolean = False)
        ...
        strSQL = strSQL & "AgtDisctVAL as Discount, r.ROE, itinerary, RCPID, t.RecID,t.srv,qty" _
                & ", NetToAL,Counter,KickBackAmt, StockCtrl"
        ...
        strSQL = strSQL & " and t.AL in (select AL from airline where City='" & myStaff.City _
                & "' and  VAT like '%" & MySession.Domain.Substring(0, 1) & "Y%') and "
        strSQL = strSQL & ParDKRCP & " order by t.RecId"
    ...
    Private Sub AddTKT2Inv(ByVal parStatus As Boolean, ByVal parNbrOfInv As Int16)
        ...
        For i As Int16 = 0 To Me.GridTKT.RowCount - 1
            ...
            End If
        Next
        GridTKT2BInv.Columns("ChargeTV").DefaultCellStyle.Format = "#,##0"
    End Sub
    Private Sub GridTKT_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridTKT.CellContentClick
        ...
        If e.ColumnIndex = 0 And
            (Me.GridTKT.Item(2, e.RowIndex).Value.ToString.Trim = "" OrElse
    ...
    Private Sub LblDocSearch_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs)
        LoadDoc()
    End Sub
        ...
        If tmpINVID > 0 Then
            ...
            If CheckBanGhiTKTNO_Exist = 0 Then
                ...
                Else
                    ...
                    cmd.CommandText = Update_INV(pubVarSRV, tblRCP.Rows(0)("CustFullName"), tblRCP.Rows(0)("CustAddress"), tblRCP.Rows(0)("CustTaxCode"),
                        tmpTTL, tmpFOP, 0, tmpINVID, tblRCP.Rows(0)("CustID"))
                End If
                cmd.ExecuteNonQuery()
            End If
    ...
    Private Sub LblPrintInv_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LblPrintInv.LinkClicked
        ...
        Try
            ...
            For i As Int16 = 1 To BaoNhieuHD
                ...
                SoHDDaBiHuy = ScalarToString("TKTNO_INVNO", "top 1 INVNO", "RCPID=" & Me.GridTKT2BInv.Item("RCPID", 0).Value &
                    " and status='XX'" &
                ...
                For j As Int16 = 0 To Me.GridTKT2BInv.RowCount - 1
                    ...
                    cmd.CommandText = " insert TKTNO_INVNO (TKNO, INVNO, INVID, RCPID, FstUser, F_VND, T_VND, C_VND, CTV_VND)" &
    ...
    Private Function UpdateTblInv(ByVal ParInvID As Integer, ByVal parHDi As Int16) As String
        ...
        If InStr("CS_LC", MyCust.CustType) > 0 Then ' voi khach CWT thi in HD cho TVTR
            Dim dtblTVTR As DataTable = GetDataTable("select VAL, VAL1, VAL2 from MISC where cat='TVCompany' and description='TVT'")
            Return Update_INV(LclSRV, dtblTVTR.Rows(0)("VAL"), dtblTVTR.Rows(0)("VAL1"),
                dtblTVTR.Rows(0)("VAL2"), TTLAmt, tmpFOP, 1, ParInvID, MyCust.CustID)
        Else
            Return Update_INV(LclSRV, Me.txtFullName.Text.Replace("--", ""), Me.txtAddress.Text.Replace("--", ""),
                Me.txtTaxCode.Text.Replace("--", ""), TTLAmt, tmpFOP, 1, ParInvID, MyCust.CustID)
        End If
    End Function
    Private Function GetFOPdetail() As String
        Dim KQ As String = ""
        Dim dtbl As DataTable = GetDataTable("select FOP, Currency, Amount, Document, ROE, Status from FOP where status in ('OK','QQ') " &
        ...
        Return KQ.Substring(1)
    End Function
    Private Sub LblGo2InvHandler_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs)
        Dim f As New InvHandler("FO")
        f.Show()
    End Sub
    Private Sub LblPreview_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs)
        Dim tmpAL As String
    ...
    Private Sub LblPreview_VisibleChanged(ByVal sender As Object, ByVal e As System.EventArgs)
    ...
    Private Sub lbkCreateE_InvDetails_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbkCreateE_InvDetails.LinkClicked
        ...
        Dim frmInv As frmE_InvEdit
        Dim strTvc As String = ScalarToString("lib.dbo.E_InvTvc78", "TVC", " City='" & myStaff.City _
                                              & "' and RcpPrefix Like '%" & Mid(TxtDocNo.Text, 1, 2) & "%'")
        Dim blnExchange As Boolean = IsExchanged(GridTKT, "StockCtrl")

        frmInv = New frmE_InvEdit(,,,, strTvc)
        frmInv.LoadDetailsBsp78(TxtDocNo.Text, strTkIds, chkIssued2TV.Checked, blnExchange)
    ...
    Public Function Search() As Boolean
        Dim strFilterRcp As String = " t.RcpNo='" & TxtDocNo.Text.Trim & "'"
        Dim tblExistingE_inv As DataTable

        If WhoIs = "FO" Then
            strFilterRcp = strFilterRcp & " and not (r.CustType='CA' and r.Counter='TVS')"
        End If

        tblExistingE_inv = GetDataTable("select DOI,MauSo,KyHieu,InvoiceNo,InvId" _
                                        & " from E_Inv78 where Status='OK' and OriFkey='' and InvId in" _
                                        & " (select InvId from E_InvLinks78 where status='OK' and RcpId=(select RecId from Rcp where Status='OK' and Rcpno='" _
                                        & TxtDocNo.Text & "'))")
        ...
        Return True
    End Function
    Private Sub lbkCurrentTS_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbkCurrentTS.LinkClicked
        TxtDocNo.Text = lbkCurrentTS.Text
        TxtDocNo.Focus()
