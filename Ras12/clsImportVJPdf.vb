...
Public Class clsImportVJPdf
    ...
    Private Function GetArrayStringPath2(xString As String, xDelimiterBegin As String, xDelimiterMidle1 As String, xDelimiterMidle2 As String, xDelimiterFinal As String) As String()
        ...
        If mStr.Contains(xDelimiterMidle1) And mStr.Contains(xDelimiterMidle2) Then
    ...
    Private Function GetAmt(xString As String, xType As Integer, xIsIndi As Boolean) As Double
        Dim mArrStr() As String
        Dim mDou As Double
        Dim mLength As Integer

        mArrStr = Split(xString)
        mLength = mArrStr.Length
        If xType = 0 Then
            If xIsIndi Then
                mDou = CDec(mArrStr(mLength - 5))
            Else
                mDou = CDec(mArrStr(mLength - 3))
            End If
        ElseIf xType = 1 Then
            If xIsIndi Then
                mDou = CDec(mArrStr(mLength - 4)) + CDec(mArrStr(mLength - 2))
            Else
                mDou = CDec(mArrStr(mLength - 2))
            End If
        Else
            mDou = CDec(mArrStr(mLength - 1))
        End If
    ...
    Public Function ParseVjPnrPdf() As Boolean
        Dim mSou, mDel, mDel2, mStr, mArrStr(), mLastTkno, mStr2, mDel3, mDel4, mStr3, mArrFly(), mArrPay(), mArrPri(), mArrCus(), mFst, mLst As String
        Dim mNumCus, mNumFly, i, mLenSeq, mSeq, j, mLength As Integer
        ...
        For i = 0 To mArrCus.Length - 1
            If (mArrCus(i).Trim <> "") Then
                ...
                If mDo Then
                    ...
                    'Ten khach, PaxType
                    mFst = Split(mStr, ",", 2)(0).Trim
                    mLst = Split(mStr, ",", 2)(1).Trim
                    If mLst.Contains("Infant:") Then
                        FlstPaxNames.Add(mFst & "/" & Split(mLst, "Infant:")(0).Trim)

                        mStr2 = Split(mLst, "Infant:")(1).Trim
                        mFst = Split(mStr2, ",")(0).Trim
                        mLst = Split(mStr2, ",")(1).Trim
                        FlstPaxNames.Add(mFst & "/" & mLst)
        ...
        For i = 0 To mArrPri.Length - 1
            If mArrPri(i).Trim <> "" Then
                ...
                mLength = mArrStr.Length
                mDo = False
                If ((mLength > 2) AndAlso (IsNumeric(mArrStr(mLength - 1)) And IsNumeric(mArrStr(mLength - 2)) And IsNumeric(mArrStr(mLength - 3)))) OrElse
                   ((mLength > 4) AndAlso (IsNumeric(mArrStr(mLength - 1)) And IsNumeric(mArrStr(mLength - 2)) And mArrStr(mLength - 3).Contains("/") And
                                           IsNumeric(mArrStr(mLength - 4)) And IsNumeric(mArrStr(mLength - 5)))) Then
                    mDo = True
                End If
                ...
                If mDo Then
                    ...
                    For j = 0 To FlstPaxNames.Count - 1
                        mFst = Split(FlstPaxNames(j), "/")(0)
                        mLst = Split(FlstPaxNames(j), "/")(1)
                        If mStr2.Contains(mFst) And mStr2.Contains(mLst) Then
                            Exit For
                        End If
                    Next
        ...
        For i = 0 To mArrPri.Length - 1
            If mArrPri(i).Trim <> "" Then
                ...
                If mDo Then
                    ...
                    If (mLength > 3) AndAlso mArrStr(mLength - 3).Contains("/") Then
                        mIndi = True
                    Else
                        mIndi = False
                    End If
                    ...
                    If mStr2.Contains("Eco") Or mStr2.Contains("SkyBoss") Or mStr2.Contains("Deluxe") Or mStr2.Contains("Add Ons") Or mStr2.Contains("Admin Fee") Or
                       mStr2.Contains("Management Fee") Or mStr2.Contains("INFANT") Then
                        ...
                        'Fare
                        FlstFares(j) = FlstFares(j) + GetAmt(mStr2, 0, mIndi)

                        'VAT Fare
                        FlstVatFares(j) = FlstVatFares(j) + GetAmt(mStr2, 1, mIndi)
                    Else
                        'Phi khac, VAT phi  khac
                        FlstPhiKhac(j) = FlstPhiKhac(j) + GetAmt(mStr2, 0, mIndi)
                        FlstVatPhiKhac(j) = FlstVatPhiKhac(j) + GetAmt(mStr2, 1, mIndi)
                    End If
