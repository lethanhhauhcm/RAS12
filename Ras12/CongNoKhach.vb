...
Public Class frmDC_Pax
    ...
    Private Sub GetPurchases()
        ...
        strSQL = String.Format("select RCPNo as OrgDocs, SRV, FstUpdate as OrgDate, Currency as OrgCurr, AMount as OrgTTLAmt, " &
            " Currency as InvCurr, Amount as InvAmt, 1.00 As InvROE, '{0}' as InvDate, '{1}' as DueDate, CustID, RecID, Nguon" &
            " from func_CC_PSP ({2},'{3}','PSP') where fstupdate >'31-dec-12'", Format(Me.txtInvoiceDate.Value, "dd-MMM-yy"),
            Format(Me.txtInvoiceDate.Value, "dd-MMM-yy"), Me.CmbCustomer.SelectedValue, CutOverDatePSP)
        strSQLACR = String.Format("select RCPNo as OrgDocs, SRV, FstUpdate as OrgDate, Currency as OrgCurr, AMount as OrgTTLAmt, " &
            " Currency as InvCurr, Amount as InvAmt, 1.00 As InvROE, '{0}' as InvDate, '{1}' as DueDate, CustID, RecID, Nguon" &
            " from func_CC_PSP ({2},'{3}','ACR') where fstupdate >'31-dec-12'", Format(Me.txtInvoiceDate.Value, "dd-MMM-yy"),
            Format(Me.txtInvoiceDate.Value, "dd-MMM-yy"), Me.CmbCustomer.SelectedValue, CutOverDatePSP)

        If Me.CmbBooker.Text <> "" Then
            strSQL = String.Format("{0} and rcpid in (select recid from rcp where ca='{1}')", strSQL, Me.CmbBooker.Text)
            strSQLACR = String.Format("{0} and rcpid in (select recid from rcp where ca='{1}')", strSQL, Me.CmbBooker.Text)
        End If
        Me.GridNo.DataSource = GetDataTable(strSQL & " union " & strSQLACR)
    ...
    Private Function XDTiGia1(ByVal parTiGiaGi As String, ByVal parFrm As String, ByVal parTo As String, ByVal ParNgayNao As Date) As Decimal
        ...
        Else
            If parFrm <> "VND" And parTo <> "VND" Then
                BankRate1 = ForEX_12(myStaff.City, ParNgayNao, parFrm, "BSR", "YY").Amount
                BankRate2 = ForEX_12(myStaff.City, ParNgayNao, parTo, "BBR", "YY").Amount
                ...
                End If
            Else
                KQ = ForEX_12(myStaff.City, ParNgayNao, parTo, "BBR", "YY").Amount
    ...
    Private Sub CmdAddPayment_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdAddPayment.Click
        ...
        If Me.CmbCNCurr.Text = "VND" Then
            BSR = 1
        Else
            BSR = ForEX_12(myStaff.City, Now, Me.CmbCNCurr.Text, "BBR", Me.CmbInvoiceAL.Text).Amount
        End If
        Try
            RecNo = Insert_KhachTra("E", Me.txtPmtDate.Value, Me.CmbCNCurr.Text, Me.CmbCNFOP.Text _
                                    ...
                                    , Me.txtPayer.Text, BSR, Me.txtCNDocNo.Text, Me.CmbPmtType.Text _
                                    , MyCust.CustID)

            If Me.CmbCNCurr.Text <> "VND" Then
                cmd.CommandText = "Update KhachTra set ROE =" & ForEX_12(myStaff.City, Me.txtPmtDate.Value, Me.CmbCNCurr.Text, "BBR", Me.CmbInvoiceAL.Text).Amount _
                    & " Where  RecID =" & RecNo
                cmd.ExecuteNonQuery()
            End If
...
ErrHandler:
    ...
    Private Function XacDinhInvUpTo() As Date
        ...
        If MyCust.KyBaoCao Is Nothing Then
            Return Now.Date
        ElseIf MyCust.KyBaoCao.Length = 0 Then
            Return Now.Date
        End If
    ...
    Private Sub BarDeleteThisPayment_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BarDeleteThisPayment.Click
        ...
        'Dim DaTra As Decimal, RollBackAmt As Decimal, GhiNoID As Int16, StrSQL As String  '^_^20230217 mark by 7643
        Dim DaTra As Decimal, RollBackAmt As Decimal, GhiNoID As Integer, StrSQL As String  '^_^20230217 modi by 7643
        ...
        For i As Int16 = 0 To dTable.Rows.Count - 1
            ...
            'StrSQL = String.Format("{0} ; Update GhiNoKhacn set Paid={1} where RecID={2}", StrSQL, DaTra - RollBackAmt, GhiNoID)  '^_^20230217 mark by 7643
            StrSQL = String.Format("{0} ; Update GhiNoKhach set Paid={1} where RecID={2}", StrSQL, DaTra - RollBackAmt, GhiNoID)  '^_^20230217 modi by 7643
        Next
        Try
    ...
    Private Sub LbLSplitPmt_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LbLSplitPmt.LinkClicked
        ...
        cmd.CommandText = String.Format("insert KhachTra (OrgDocs, OrgCurr, OrgAmt, OrgDate, CustID, CustShortName, CustName, FOP, " &
            "ReceiveBy, FstUser, Description, Note, PmtType, POS, ROE, LstPmt, LinkID) select OrgDocs, OrgCurr, {0}, OrgDate, " &
            "CustID, CustShortName, CustName, FOP, ReceiveBy, '{1}', Description, Note, PmtType, POS, ROE, LstPmt,{2} from Khachtra " &
            "where recid={3}; Update KhachTra set OrgAmt=Used, LinkID={2} where recid={3}", Me.GridDungTienNao.CurrentRow.Cells("Balance").Value,
    ...
    Private Sub LblListDetail_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LblListDetail.LinkClicked
        InHoaDon(Application.StartupPath, "R12_CC_TRXDetailByProfINV.xlt", "F", "YY", Now, Now _
                 , Me.GridTraChoKhoanNao.CurrentRow.Cells("RecID").Value, "YY", MySession.Domain _
    ...
    Private Sub LblTracing_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblTracing.LinkClicked
        ...
        cmd.CommandText = "select sum(invamt) from GhiNoKhach where " & strDK
        KQ = cmd.ExecuteScalar
        Me.TxtDaINV.Text = Format(KQ, "#,##0.0")
        cmd.CommandText = "select sum(orgAmt*ROE) from KhachTra where pmtType='PSP' and " & strDK
        KQ = cmd.ExecuteScalar
    ...
    Private Sub GridPendingINV_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridPendingINV.CellContentClick
        ...
        LoadGridVatInv()
    End Sub
    Private Function LoadGridVatInv() As Boolean
        Dim tblVatInv As DataTable = GetVatInvoices(GridPendingINV.CurrentRow.Cells("CustShortName").Value _
                                        , "", "", " and GhiNoId=" & GridPendingINV.CurrentRow.Cells("RecId").Value)

        dgrVatInv.DataSource = tblVatInv
    End Function
    ...
    Private Sub lbkLinkVatInv_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbkLinkVatInv.LinkClicked
        Dim tblVatInv As DataTable = GetVatInvoices(GridPendingINV.CurrentRow.Cells("CustShortName").Value _
                                        , "", "", " and GhiNoId=0")
        Dim frmSelect As New frmShowTableContent(tblVatInv, "Select VAT Invoce", "RecId")
        frmSelect.dgrInput.Columns("Amount").DefaultCellStyle.Format = "#,##0"
        frmSelect.dgrInput.SelectionMode = DataGridViewSelectionMode.FullRowSelect
        If frmSelect.ShowDialog = DialogResult.OK Then
            Dim lstQuerries As New List(Of String)
            For Each objRow As DataGridViewRow In frmSelect.dgrInput.SelectedRows
                lstQuerries.Add("Update VatInv set GhiNoId=" & GridPendingINV.CurrentRow.Cells("RecId").Value _
            ...
            If UpdateListOfQuerries(lstQuerries, Conn) Then
                LoadGridVatInv()
            Else
                MsgBox("Unable to link VAT Invoces!")
            End If

        End If
    End Sub

    Private Sub lbkLinkVatInv4Pmt_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbkLinkVatInv4Pmt.LinkClicked
        Dim decMaxAmount As Decimal = GridDungTienNao.CurrentRow.Cells("OrgAmt").Value - ScalarToDec("VatInvAmts", "Total", "Status='ok'" _
                                        & " And InvId in(Select InvId from VatInv where Status='OK'" _
                                        & " and KhachTraId=0 and CustShortName='" & GridDungTienNao.CurrentRow.Cells("CustShortName").Value & "')")
        Dim frmSelect As New frmSelectVatInv(GridDungTienNao.CurrentRow.Cells("CustShortName").Value, decMaxAmount, "InvId")
        With frmSelect.dgrVatInv
            .Columns("CustFullName").Visible = False
            .Columns("TaxCode").Visible = False
            .Columns("Address").Visible = False
        End With
        If frmSelect.ShowDialog = DialogResult.OK Then
            Dim lstQuerries As New List(Of String)
            For Each objRow As DataGridViewRow In frmSelect.dgrVatInv.SelectedRows
                lstQuerries.Add("Update VatInv set KhachTraId=" & GridDungTienNao.CurrentRow.Cells("RecId").Value _
                                & " ,ClearDate=getdate()" _
                                & " where RecId=" & objRow.Cells("RecId").Value)
            Next

            If UpdateListOfQuerries(lstQuerries, Conn) Then
                MsgBox(frmSelect.dgrVatInv.SelectedRows.Count & " VAT invoices are cleared!")
            Else
                MsgBox("Unable to link VAT Invoces!")
            End If
        ...
        End If
    End Sub
    Private Sub lbkViewInvDetails_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbkViewInvDetails.LinkClicked
        If GridDungTienNao.CurrentRow Is Nothing Then Exit Sub
        Dim strQuerry As String = "select i.CustShortName" _
            & ",i.InvoiceNo ,i.ClearDate,sum(a.Total) as InvAmount,l.RcpTcode" _
            & "  from ras12.dbo.vatinv i" _
            & " left join khachtra k on i.khachtraid=k.RecID" _
            & " left join VatInvAmts a on i.InvID=a.InvID" _
            & " left join VatInvLinks l on i.InvID=l.VatInvID" _
            & " where i.khachtraId=" & GridDungTienNao.CurrentRow.Cells("RecId").Value _
            & " group by i.KhachTraId,i.CustShortName,k.OrgAmt,k.OrgDate,i.InvoiceNo ,i.ClearDate,RcpTcode"

        Dim tblInv As DataTable
        tblInv = GetDataTable(strQuerry, Conn)
        Dim frmShow As New frmShowTableContent(tblInv, "VAT Invoice Details")
        frmShow.ShowDialog()
    End Sub
End Class