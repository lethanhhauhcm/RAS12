...
Public Class SendToEmail
    ...
    Private Sub LoadGridEmail(RMK As String)
        GridEmail.DataSource = GetDataTable("select RecID, CAT, VAL, Val1 as Notify, Details as Email , RMK as City , LstRun, convert(varchar,LstRun, 103) as LstUpdate from FT.dbo.MISC where CAT='EMAILTO' and RMK = '" & RMK & "' order by cast(VAL as int) desc")
        GridEmail.Columns("VAL").Width = "35"
        GridEmail.Columns("City").Width = "35"
        GridEmail.Columns("Notify").Width = "275"
        GridEmail.Columns("LstUpdate").Width = "110"
    ...
    Private Sub GridEmail_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles GridEmail.CellClick
        ...
        If tbVal.Text.ToString() = "" Or tbVal.Text.ToString() <> GridEmail.Rows(Index).Cells("Val").Value.ToString() Then
            ...
            tbRecID.Text = GridEmail.Rows(Index).Cells("RecID").Value.ToString()
        Else
...
#Enable Warning RE0001 ' Regex issue: {0}
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        Else
            Dim cmd As SqlClient.SqlCommand = Conn.CreateCommand
            cmd.CommandText = "delete FT.dbo.MISC where RecID = @RecID"
            cmd.Parameters.Add("@RecID", SqlDbType.Int).Value = Integer.Parse(tbRecID.Text.ToString())
            cmd.ExecuteNonQuery()
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        End If
        Dim cmd As SqlClient.SqlCommand = Conn.CreateCommand
            cmd.CommandText = "update FT.dbo.MISC Set Val1 = @Notify,Details = @Details, LstRun = getdate(), Val = @Val where RecID = @RecID"
            cmd.Parameters.Add("@Details", SqlDbType.VarChar).Value = TbDetails.Text.ToString()
            cmd.Parameters.Add("@RecID", SqlDbType.Int).Value = Integer.Parse(tbRecID.Text.ToString())
            cmd.Parameters.Add("@Notify", SqlDbType.VarChar).Value = TbNotify.Text.ToString()
            cmd.Parameters.Add("@Val", SqlDbType.VarChar).Value = tbVal.Text.ToString()
            cmd.ExecuteNonQuery()
        MsgBox("Đã chỉnh sửa")
        LoadGridEmail(CbbWhereEmail.Text)
        tbVal.Text = ""
