...
Public Class Sales
    ...
    Private XlsProcess As New Process
    ...
    Private Sub Sales_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ElseIf DoWhat = "RPT" Then
            ...
            Dim strSQL As String = "select RecID as VAL, CustShortName as DIS from Customerlist" _
                    & " where recid >0 And Status='OK' and City='" & myStaff.City & "'"
    ...
    Private Sub LoadData_S(ByVal parAL As String)
        Dim strSQL As String = "select RecID, AL, Raw, ADJ from FB_RBD_QT where cat='" & DoWhat & "'" &
            " and AL='" & parAL & "'"
    ...
    Private Sub CmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CmdSave.Click
        For r As Int16 = 0 To Me.GridFB_RBD.RowCount - 1
            cmd.CommandText = "Update FB_RBD_QT set ADJ='" & Me.GridFB_RBD.Item("ADJ", r).Value & "'," &
    ...
    Private Sub LblRun_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LblRun.LinkClicked
        ...
        objWbk = objExcel.Workbooks.Add(My.Application.Info.DirectoryPath & "\R12_SalesPerformance.xltm")
        objExcel.Visible = True
        objWsh = objWbk.Sheets("Para")

        objWsh.Range("B1").Value = My.Application.Info.DirectoryPath
        objWsh.Activate()

        'XlsProcess = Process.Start(My.Application.Info.DirectoryPath & "\R12_SalesPerformance.xlt")
        'XlsProcess = Process.Start("X:\Xls from SQL\R12_SalesPerformance.xlt")
        Me.StatusFeedback.Text = ""
        Me.LblRun.Enabled = True
        'LogReport("RAS", "R12_SalesPerformance", myStaff.SICode, myStaff.City)
    End Sub
    ...
    Private Sub TaoDataToTmpTable(pStep As Int16, pFrom As Date, pThru As Date, pAL As String, pAgt As String, pTblName As String)
        ...
        cmd.Parameters.Add("@Thru", SqlDbType.DateTime).Value = CreateToDate(pThru)
        cmd.Parameters.Add("@AL", SqlDbType.VarChar).Value = pAL
        cmd.Parameters.Add("@Agt", SqlDbType.VarChar).Value = pAgt

        cmd.ExecuteNonQuery()

        'Update CustLocation là 'HAN' cho các CustShortName không thuộc SGN
        If myStaff.City = "SGN" Then
            cmd.CommandText = "update " & pTblName _
            & " set CounterLocation='HAN'" _
            & " WHERE CustShortName IN " _
            & " (SELECT DISTINCT CUSTSHORTNAME" _
            & " From ReportData_BSP" _
            & " Where CustShortName Not In (Select CustShortName FROM CustomerList)" _
            & " And SUBSTRING(CUSTSHORTNAME,1,3)='HAN')"
            cmd.ExecuteNonQuery()
        End If
