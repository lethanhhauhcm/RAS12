...
Module mdlE_Invoice
    ...
    Public pstrSerialCert123 As String = "540101040F7FD8666CF54AB5E6CB9AF7"
    ...
    Structure FunctionName
        ...
        Const GetHashInvNoticeErrors = "GetHashInvNoticeErrors"
        ...
        Const SendInvNoticeErrorsWidthToken = "SendInvNoticeErrorsWidthToken"
    ...
    Public Function GetE_InvConnects(strBiz As String, strCity As String, Optional strTvc As String = "") As List(Of clsE_InvConnect)
        ...
        Dim strQuerry As String = "select distinct biz,tvc,City from lib.dbo.E_InvSettings78" _
    ...
    End Function
    Public Function SingleUpLoadCustomer2VNPT(intCustid As Integer)
        ...
        lstWsConnects = GetE_InvConnects("PAX", myStaff.City)


        strQuerry = "Select * from Lib.dbo.Customer" _
            & " where RECID =" & intCustid

        tblCustomer = GetDataTable(strQuerry, Conn)
        For Each objRow As DataRow In tblCustomer.Rows
            ...
            UpLoadCustomer2VNPT(lstWsConnects, lstCustData, objRow("RecId"))
        Next
        Return True
    End Function
    ...
    Public Function CreateInvProduct(objProduct As clsProduct, Optional blnTT78 As Boolean = False) As XElement
        ...
        If Not blnTT78 Then
            decProductPrice = Math.Abs(decProductPrice)
        End If
    ...
    Public Function CreateInvProductTt78(objProduct As clsProduct) As XElement
        ...
        '<TSuat>KKKNT</TSuat>
        '
        Dim objTTKhac As XElement = <TTKhac>
                                        <TTin>
    ...
    Public Function CreateInvData(strCustId As String, strCustFullName As String _
        ...
        '    <AmountInWords><%= TienBangChu(Math.Abs(decTotalInv)) & " đồng" %></AmountInWords>
        '</Invoice>
        If intVatDiscount <> 0 AndAlso Not pblnTT78 Then
            objVatDiscount =
                <Extra><%= 100 - intVatDiscount %></Extra>
            objInvoice.Add(objVatDiscount)
        End If
    ...
    Public Function ViewInv(strTvc As String, blnTt78 As Boolean, blnNoOriInv As Boolean _
                            , strMauSo As String, strKyHieu As String, intInvNo As Integer _
                            , strFkey As String) As Boolean
        Dim strHtml As String = ""
        If intInvNo = 0 Then
            strHtml = GetInvHtmlByFkey(strTvc, blnTt78, blnNoOriInv, strFkey)
        Else
            strHtml = GetInvHtmlByInvNo(strTvc, blnTt78, blnNoOriInv, strMauSo, strKyHieu, intInvNo)
        End If
        If strHtml = "" Then
            MsgBox("Unable to view E Invoice!")
            Return False
        Else
            Dim frmShow As New frmShowHtml(strHtml)
            frmShow.ShowDialog()
            Return True
        End If
    End Function
    ...
    Public Function GetOriginalInv(blnTt78 As Boolean, strTkno As String) As DataRow
        ...
        Dim objOriInv As DataRow
        Dim strInvTable As String = "lib.dbo.E_inv"
        Dim strInvDetailTable As String = "lib.dbo.E_invDetails"
        ...
        End If
        tblOldTkts = GetDataTable("select top 1 RcpId,RecId,Tkno from Tkt" _
                                  & " where SRV='S' and replace(Tkno,' ','')='" _
                                                     & strTkno & "' order by RecId desc")
        tblOldInv = GetDataTable("select * from " & strInvDetailTable & " d" _
                                 ...
                                 & strTkno & "%') order by i.InvId desc", Conn)
        ...
        Select Case tblOldInv.Rows.Count
            ...
            Case 1
                ...
                strOriFkey = tblOldInv.Rows(0)("OriFkey")
            Case Else
                Dim frmSelect As New frmSelectInvoice(tblOldInv, "Hãy chọn hóa đơn gốc phù hợp", "OriFkey")
                If frmSelect.ShowDialog = DialogResult.OK Then
                    objOriInv = frmSelect.SelectedDataRow
                    strOriFkey = frmSelect.SelectedValue
                Else
                    MsgBox("Bạn đã bỏ không chọn hóa đơn gốc!")
                    Return Nothing
                End If
        End Select
        ...
        Do While strOriFkey <> ""
            tblOldInv = GetDataTable("select  i.* from " & strInvDetailTable & " d" _
                                 & " left join " & strInvTable & " i on d.InvId=i.InvId" _
                                 & " where i.Srv='S' and i.InvId=" & tblOldInv.Rows(0)("OriFkey"), Conn)
            Select Case tblOldInv.Rows.Count
                Case 0
                    Return Nothing
                Case 1
    ...
    Public Function ApproveDraftInv(blnITT78 As Boolean, strTvc As String, strMauSo As String _
        ...
        Dim strInvoiceTable As String = "E_Inv78"
        Dim objE_Inv As New clsE_Invoice
        ...
        If strSerialCert = "" Then
            If Not objE_Inv.PublishInvFkey(objConnect.WsUrl, objConnect.AccountName, objConnect.AccountPass _
                ...
                MsgBox("Unable to Approve Draft E Invoice!" & vbNewLine & objE_Inv.ResponseDesc)
                Return False
            End If
        Else
            Dim objInvToken As clsInvToken
            objInvToken = objE_Inv.getHashInv(objConnect.WsUrl, objConnect.UserName, objConnect.UserPass _
                                               , objConnect.AccountName, objConnect.AccountPass _
                                               , pstrSerialCert123, strFkey, strMauSo)
            If objInvToken Is Nothing Then
                MsgBox("Unable to get Invoice Token!" & vbNewLine & objE_Inv.ResponseDesc)
                Return False
            End If
            If Not objE_Inv.publishInvWithToken(objConnect.WsUrl _
                                        , objConnect.UserName, objConnect.UserPass _
                                        , objConnect.AccountName, objConnect.AccountPass _
                                        , strSerialCert, objInvToken, strMauSo, strKyHieu) Then
                MsgBox("Unable to Approve Draft E Invoice!" & vbNewLine & objE_Inv.ResponseDesc)
                Return False
            End If
        ...
        If objE_Inv.ResponseDesc.StartsWith("OK") Then
            ...
            For Each strInvoiceId_No As String In arrResults
                ...
                If ExecuteNonQuerry("Update " & strInvoiceTable & " set Draft='False', InvoiceNo=" _
                                    & arrBreaks(1) & ",DOI=Getdate()" _
            ...
            Return True
        Else
            MsgBox("Unable to Approve Draft E Invoice!")
            Return False
        End If
    ...
    Public Function DeleteDraftInv(blnTt78 As Boolean, strTvc As String, strFkey As String) As Boolean
        ...
        Return True
    End Function
    Public Function MergeOldNewInvoice(strTVC As String) As Boolean

        Select Case strTVC
            Case "TVPM SGN", "APG", "VLM"
                Return True
            Case Else
                Return False
        End Select
    End Function
    Public Function NoOriInv(strFkey As String, strTvc As String) As Boolean
        If IsNumeric(strFkey) AndAlso CInt(strFkey) > 40000 Then
            Return False
        Else
            Return Not MergeOldNewInvoice(strTvc)
        End If
    End Function
    Public Function GetPatternShortForm(strPattern As String, strFkey As String) As String
        If IsNumeric(strFkey) AndAlso CInt(strFkey) > 40000 Then
            Return Mid(strPattern, 1, 1)
        Else
            Return strPattern
        End If
    End Function
