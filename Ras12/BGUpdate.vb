...
Public Class BGUpdate
    ...
    Private mblnFormLoaded As Boolean
    ...
    Private Sub LblUpdate_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles LckLblUpdate.LinkClicked
        ...
        cmd.CommandText = "insert into BG (CustId, CustShortName, ContractNo, ContractValidity, ContractOrgCopy, " &
                "BGAMount, BGValidFrm, BGExpireDate, Bank, BGOrgCopy, Remark, TVCompany, FstUser,City) values (" &
                "@CustId, @CustShortName, @ContractNo, @ContractValidity, @ContractOrgCopy, " &
                "@BGAMount, @BGValidFrm, @BGExpireDate, @Bank, @BGOrgCopy, @Remark, @TVCompany, @FstUser, @City)"
        ...
        If CmbBank.Text <> "CRD" AndAlso CmbBank.Text <> "CSH" Then
            SaleLog.CreatePointmentItem(CmbCustomer.Text & " BG will expired in 15 days" _
                                    , "RAS", Me.TxtValidThru.Value.Date.AddDays(-15).AddHours(8) _
                                    , Me.TxtValidThru.Value.Date.AddDays(-15).AddHours(9))
            SaleLog.CreatePointmentItem(CmbCustomer.SelectedValue & " BG will expired in 7 days" _
                                    , "RAS", Me.TxtValidThru.Value.Date.AddDays(-7).AddHours(8) _
                                    , Me.TxtValidThru.Value.Date.AddDays(-7).AddHours(9))
        End If
    ...
    Private Sub LoadGridBG()
        ...
        If Not mblnFormLoaded Then Exit Sub
        ...
        If Me.chkExpiredIn7Days.Checked Then
            strSQL = strSQL & " and status='OK' and  BGExpireDate between '" & CreateFromDate(Now) _
                & "' and '" & CreateToDate(Now.AddDays(7)) & "'"
        End If
        ...
        End If
        strSQL = strSQL & " order by CustShortName,ValidFrm"
        LoadDataGridView(GridBG, strSQL, Conn)
    ...
    Private Sub LckLblApprove_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles LckLblApprove.LinkClicked
        ...
        With GridBG.CurrentRow
            SaleLog.CreatePointmentItem(.Cells("CustShortName").Value & " BG will expired in 15 days" _
                                    , "RAS", .Cells("ExpireDate").Value.AddDays(-15).AddHours(8) _
                                    , .Cells("ExpireDate").Value.AddDays(-15).AddHours(9))
            SaleLog.CreatePointmentItem(.Cells("CustShortName").Value & " BG will expired in 7 days" _
                                    , "RAS", .Cells("ExpireDate").Value.AddDays(-7).AddHours(8) _
                                    , .Cells("ExpireDate").Value.AddDays(-7).AddHours(9))
        End With
    ...
    Private Sub chkExpiredIn7Days_Click(sender As Object, e As EventArgs) Handles chkExpiredIn7Days.Click
        LoadGridBG()
    End Sub

    Private Sub OptCredit_CheckedChanged(sender As Object, e As EventArgs) Handles OptCredit.CheckedChanged
        LoadGridBG()
    End Sub

    Private Sub OptDeposit_CheckedChanged(sender As Object, e As EventArgs) Handles OptDeposit.CheckedChanged
        LoadGridBG()
    End Sub
